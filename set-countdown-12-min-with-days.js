...
async function setCountdown() {
  try {
    console.log('🔄 Устанавливаем обратный отсчёт на 1 день 12 минут...\n');
    
    // Устанавливаем countdown_end на 1 день + 12 минут вперёд
    const countdownEnd = new Date(Date.now() + (24 * 60 * 60 * 1000) + (12 * 60 * 1000)); // 1 день + 12 минут
    ...
    if (result.rows.length > 0) {
      ...
      console.log('\n✨ Теперь будет показываться 4 карточки: Дней, Часов, Минут, Секунд');
    } else {
