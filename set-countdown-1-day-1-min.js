...
async function setCountdown() {
  try {
    console.log('🔄 Устанавливаем обратный отсчёт на 1 день 1 минуту...\n');
    
    // Устанавливаем countdown_end на 1 день + 1 минуту вперёд
    const countdownEnd = new Date(Date.now() + (24 * 60 * 60 * 1000) + (1 * 60 * 1000)); // 1 день + 1 минута
    ...
    if (result.rows.length > 0) {
      ...
      console.log('\n✨ Будет показываться 4 карточки: Дней, Часов, Минут, Секунд');
    } else {
