...
import { ArrowUpRight, ArrowDownRight, TrendingUp, Users, Zap, Clock, Search, Filter, ArrowLeft, User, Calendar, DollarSign } from "lucide-react"
...
export default function AllActivityPage() {
  ...
  const getActivityText = (activity: Activity) => {
    ...
    const amount = activity.amount?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    
    switch (activity.type) {
      case "deposit":
        return {
          main: `пополнил баланс`,
          amount: `${config.sign}$${amount}`,
          detail: "💳 Пополнение счета"
        }
      case "withdrawal":
        return {
          main: `вывел средства`,
          amount: `${config.sign}$${amount}`,
          detail: "📤 Вывод на кошелек"
        }
      case "investment":
        return {
          main: `создал инвестицию`,
          amount: `$${amount}`,
          detail: `💎 План: ${activity.plan_name || "Не указан"}`
        }
      case "referral":
        return {
          main: `получил реферальный доход`,
          amount: `${config.sign}$${amount}`,
          detail: "👥 Доход с рефералов"
        }
      case "profit":
        return {
          main: `получил прибыль`,
          amount: `${config.sign}$${amount}`,
          detail: "📈 Прибыль от инвестиций"
        }
      default:
        return {
          main: `совершил операцию`,
          amount: `$${amount}`,
          detail: "⚡ Операция"
        }
    }
  ...
  const incomeActivities = filteredActivities.filter(a => ['deposit', 'profit', 'referral'].includes(a.type))
  const expenseActivities = filteredActivities.filter(a => ['withdrawal'].includes(a.type))
  const investmentActivities = filteredActivities.filter(a => a.type === 'investment')

  const totalIncome = incomeActivities.reduce((sum, a) => sum + (a.amount || 0), 0)
  const totalExpense = expenseActivities.reduce((sum, a) => sum + (a.amount || 0), 0)
  const totalInvestment = investmentActivities.reduce((sum, a) => sum + (a.amount || 0), 0)
      ...
      <div className="container mx-auto max-w-7xl relative z-10 pt-20 pb-20 px-4">
        ...
        >
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 text-center">
            <div className="text-3xl font-bold text-cyan-400 mb-2">{totalActivities}</div>
          ...
          <div className="bg-emerald-500/10 backdrop-blur-sm border border-emerald-500/30 rounded-2xl p-6 text-center">
            ...
            <div className="text-emerald-300 text-sm">💰 Доходы ({incomeActivities.length})</div>
          </div>
          <div className="bg-red-500/10 backdrop-blur-sm border border-red-500/30 rounded-2xl p-6 text-center">
            ...
            <div className="text-red-300 text-sm">📤 Расходы ({expenseActivities.length})</div>
          </div>
          <div className="bg-blue-500/10 backdrop-blur-sm border border-blue-500/30 rounded-2xl p-6 text-center">
            <div className="text-3xl font-bold text-blue-400 mb-2">
              ${totalInvestment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
            <div className="text-blue-300 text-sm">💎 Инвестиции ({investmentActivities.length})</div>
          </div>
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 text-center">
            <div className="text-3xl font-bold text-purple-400 mb-2">{todayActivities}</div>
            <div className="text-slate-300 text-sm">Сегодня</div>
          </div>
        </motion.div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-3 rounded-2xl bg-gradient-to-r ${config.gradient} text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                        <span className="text-xl">{config.icon}</span>
                      </div>
                      <Badge className={`${config.textColor} bg-white/10 border-white/20 font-medium px-3 py-1`}>
                        {config.name}
                      </Badge>
                    </div>
