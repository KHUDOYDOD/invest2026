...
async function testUpdateStatistics() {
  try {
    console.log('🧪 Тестируем функцию updateStatistics...\n');
    
    // Показываем текущие данные
    console.log('📊 Текущие данные из базы:');
    const [usersResult, investmentsResult, payoutsResult] = await Promise.all([
      query('SELECT COUNT(*) as count FROM users'),
      query(`SELECT COALESCE(SUM(amount), 0) as total_amount FROM transactions WHERE type = 'investment' AND status = 'completed'`),
      query(`SELECT COALESCE(SUM(amount), 0) as total_amount FROM transactions WHERE type = 'withdrawal' AND status = 'completed'`)
    ]);

    console.log(`   Пользователи: ${usersResult.rows[0].count}`);
    console.log(`   Инвестиции: $${investmentsResult.rows[0].total_amount}`);
    console.log(`   Выплаты: $${payoutsResult.rows[0].total_amount}`);
